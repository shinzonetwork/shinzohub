...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=shinzo \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=shinzod \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/shinzod ./cmd/shinzod
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/shinzod.exe ./cmd/shinzod
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/shinzod
...
draw-deps:
	...
	@goviz -i ./cmd/shinzod -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c shinzo --local -f chains.yaml
endif
...
set-testnet-configs:
	shinzod config set client chain-id localchain_9000-1
	shinzod config set client keyring-backend test
	shinzod config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | shinzod keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | shinzod keys add acc1 --recover`
