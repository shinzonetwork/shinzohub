...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=shinzohub \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=shinzohubd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/shinzohubd ./cmd/shinzohubd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/shinzohubd.exe ./cmd/shinzohubd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/shinzohubd
...
draw-deps:
	...
	@goviz -i ./cmd/shinzohubd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c shinzohub --local -f chains.yaml
endif
